...
{
protected:
    shared_ptr<BaseControlDevice> _controlDevices;
...
protected:
	...
	void InternalSetStateFromInput() override
	{
		for(KeyMapping& keyMapping : _keyMappings) {
			...
			bool turboOn = 0;//(uint8_t)(_emu->GetFrameCount() % turboFreq) < turboFreq / 2;
			if(turboOn) {
...
public:
	...
	PceController(uint8_t port, KeyMappingSet keyMappings) : BaseControlDevice(ControllerType::PceController, port, keyMappings)
	{
