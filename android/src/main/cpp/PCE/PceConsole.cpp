...
bool PceConsole::LoadRom(vector<uint8_t> romData)
{
...
//	}
	...
	_psg.reset(new PcePsg(this));
	_memoryManager.reset(new PceMemoryManager(this, _vpc.get(), _vce.get(), _controlManager.get(), _psg.get(), _timer.get(), _mapper.get(), romData, cardRamSize, false));
	_cpu.reset(new PceCpu(_memoryManager.get()));
...
void PceConsole::SaveBattery()
{
 ...
 PceControlManager* PceConsole::GetControlManager()
 {
...
uint8_t* PceConsole::GetPpuFrame()
{
	// PpuFrameInfo frame = {};
	...
	return (uint8_t*)_vpc->GetScreenBuffer();
