...
//	PcEngineConfig& cfg = _emu->GetSettings()->GetPcEngineConfig();
	while(clocksToRun >= 6) {
		...
		for(int i = 0; i < 6; i++) {
			...
			ch.Run(minTimer);
			leftOutput += (int32_t)ch.GetOutput(true, _state.LeftVolume) * (int32_t)1;//(int32_t)cfg.ChannelVol[i] / 100;
			rightOutput += (int32_t)ch.GetOutput(false, _state.RightVolume) * (int32_t)1;//(int32_t)cfg.ChannelVol[i] / 100;
		}
...
{
	uint8_t offset = 0;//_emu->GetSettings()->GetPcEngineConfig().UseHuC6280aAudio ? 0x10 : 0;
	for(int i = 0; i < 6; i++) {
...
//    SoundManager::SetAudioBuffer(_soundBuffer, sampleCount);

    _console->_soundManager->WriteToBuffer((uint8_t*)_soundBuffer,sampleCount*2);
	_clockCounter = 0;
